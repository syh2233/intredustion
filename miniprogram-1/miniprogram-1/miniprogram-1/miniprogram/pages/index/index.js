Page({
  ...
  loadLocalStorage() {
    try {
      ...
      // 确保 alarmRecords 是数组
      const safeAlarmRecords = Array.isArray(alarmRecords) ? alarmRecords : [];

      this.setData({
        tempThreshold: savedTemp,
        smokeThreshold: savedSmoke,
        alarmRecords: safeAlarmRecords,
        unhandledCount: safeAlarmRecords.filter(r => !r.handled).length
      });
  ...
  controlServo() {
    ...
    // 通过舵机控制API发送命令到ESP32
    wx.request({
      url: `${this.data.backendUrl}/api/servo/control`,
      method: 'POST',
      timeout: 10000,
      header: {
        'content-type': 'application/json'
